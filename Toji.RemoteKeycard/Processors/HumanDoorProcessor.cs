...
namespace Toji.RemoteKeycard.Processors
{
    public class HumanDoorProcessor : BaseDoorProcessor
    {
        ...
        protected override bool ProcessGate(Gate gate, Player player) => !gate.IsLocked && (player.CheckPermissions(KeycardPermissions.ExitGates) || player.Role is { Team: Team.FoundationForces, Type: not RoleTypeId.FacilityGuard });
        ...
        protected override bool ProcessNotKeycard(BasicDoor door, Player player) => !player.IsCuffed && !player.HasEffect<SeveredHands>();
    }
}
