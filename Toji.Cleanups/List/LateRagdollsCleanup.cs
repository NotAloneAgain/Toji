...
namespace Toji.Cleanups.List
{
    public class LateRagdollsCleanup : RagdollCleanup
    {
        ...
        public override void Cleanup(List<Ragdoll> ragdolls, List<Player> players, out float cooldown)
        {
            ...
            {
                if (ragdoll == null || RoleExtensions.GetTeam(ragdoll.Role) == Team.SCPs || ragdoll.ExistenceTime <= 15)
                {
                    continue;
                }
                ...
                if (ragdoll.GameObject.IsValid())
                {
                    ...
                    if (hasMimicry && (BaseSubclass.TryGet(ragdoll, out _) || ragdoll.ExistenceTime <= 144))
                    {
                        continue;
                    }
                }
